...
export function renderReport(events: RunEvent[]): string {
  ...
  const lines: string[] = [
    ...
    "## Issues",
    "",
    "| Issue | Status | Attempts | Duration | Files touched |",
    "| ----- | ------ | -------- | -------- | ------------- |",
  ];
  for (const r of sortedRows) {
    lines.push(
      `| ${r.title} | ${r.status} | ${r.attempts ?? "–"} | ${formatDuration(r.durationMs)} | ${
        r.filesTouched.join(", ") || "–"
      } |`,
    );
  }
  ...
  return {
    ...
    onEvent(event) {
      ...
      if (event.type === "backend-result") {
        const file = join(dir, `issue-${nn(event.issue)}-attempt-${event.attempt}.txt`);
        writeFileSync(
          file,
